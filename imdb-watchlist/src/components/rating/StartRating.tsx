...
const StarRating: React.FC<StarRatingProds> = ({ maxStarts = 5 }) => {
      ...
      <div style={startContainerStype}>
        {Array.from({ length: 5 }, (_, i) => (
          <Start
            key={i}
            startNum={i + 1}
            onHover={(val) => setCurrentRating(val)}
            full={i + 1 <= currentRating}
          />
        ))}
      </div>
...
function Start({ onHover, startNum, full }: StarProds) {
  return (
    <span
      role="button"
      style={startStyle}
      onMouseEnter={() => onHover(startNum)}
    >
      {full ? (
        <svg
